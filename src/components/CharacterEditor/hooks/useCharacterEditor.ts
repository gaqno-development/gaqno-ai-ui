...
export const useCharacterEditor = (bookId: string, characterId?: string) => {
  ...
  const updateCharacterDetails = useCallback((partial: Partial<ICharacterDetails>) => {
  ...
  const handleAnalyzeCharacter = useCallback(async () => {
    ...
    try {
      ...
      addNotification({
        ...
        message: err instanceof Error ? err.message : 'Não foi possível analisar o personagem.',
        duration: 5000,
      });
    } finally {
  ...
  const handleGenerateAvatar = useCallback(async () => {
    ...
    try {
      ...
      addNotification({
        ...
        message: err instanceof Error ? err.message : 'Não foi possível gerar o avatar.',
        duration: 5000,
      });
    } finally {
  ...
  const handleSave = useCallback(async () => {
    ...
    const metadata = { ...character?.metadata, characterDetails: characterDetails || undefined };
  ...
  }, [characterId, character?.metadata, characterDetails, name, description, avatarUrl, updateCharacter, addNotification]);
