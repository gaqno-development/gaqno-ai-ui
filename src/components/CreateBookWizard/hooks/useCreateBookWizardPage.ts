...
export const useCreateBookWizardPage = () => {
  ...
  const handleGenerateCompleteBlueprint = useCallback(async () => {
    ...
    try {
      ...
      if (Array.isArray(settingsArray) && settingsArray.length > 0) {
        const newSettings = settingsArray.map((s: Record<string, unknown>, idx: number) => {
      ...
      if (Array.isArray(charactersData) && charactersData.length > 0) {
        const newCharacters = charactersData.map((char: Record<string, unknown>, idx: number) => {
      ...
      if (Array.isArray(itemsArray) && itemsArray.length > 0) {
        const newItems = itemsArray.map((item: Record<string, unknown>, idx: number) => ({
          id: `temp-${Date.now()}-${idx}`,
      ...
      if (context.target_audience) form.setValue('target_audience', context.target_audience as string);
      if (context.central_themes) form.setValue('central_themes', context.central_themes as string);
      else if (Array.isArray(context.themes)) form.setValue('central_themes', (context.themes as string[]).join(', '));
      ...
      addNotification({
        ...
        message: 'Todos os steps foram preenchidos. Navegue entre os steps para revisar e editar.',
        duration: 5000,
      });
    } catch (err: unknown) {
      addNotification({
        ...
        message: err instanceof Error ? err.message : 'Não foi possível gerar o blueprint completo.',
        duration: 5000,
      });
    }
  ...
  const handleSaveDraft = useCallback(async () => {
    ...
    try {
      saveDraft();
      addNotification({ type: 'success', title: 'Rascunho salvo', message: 'Seu progresso foi salvo localmente.', duration: 3000 });
    } catch (err: unknown) {
      addNotification({
        type: 'error',
        title: 'Erro ao salvar rascunho',
        message: err instanceof Error ? err.message : 'Não foi possível salvar o rascunho.',
        duration: 5000,
      });
    } finally {
      setIsSaving(false);
      ...
      await Promise.all([
        ...
        ...wizard.characters.map((c) =>
          booksMutations.createCharacter.mutateAsync({
            bookId,
            data: { name: c.name, description: c.description || null, avatar_url: null, metadata: c.role ? { role: c.role } : {} },
          }),
        ),
      ]);
      clearDraft();
      addNotification({ type: 'success', title: 'Livro criado com sucesso!', message: 'Redirecionando...', duration: 3000 });
      setTimeout(() => navigate(`/ai/books/${bookId}`), 500);
    } catch (err: unknown) {
      addNotification({
        type: 'error',
        title: 'Erro ao criar livro',
        message: err instanceof Error ? err.message : 'Ocorreu um erro inesperado. Tente novamente.',
        duration: 5000,
      });
      setIsCreatingBook(false);
    }
  }, [formValues, selectedGenre, user, wizard, booksMutations, settingsMutations, itemsMutations, toneStyleMutations, clearDraft, addNotification, navigate]);
