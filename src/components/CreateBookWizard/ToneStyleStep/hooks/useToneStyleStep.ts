...
export function useToneStyleStep({ bookContext }: IToneStyleStepProps) {
  ...
  const handleGenerateTone = async (): Promise<string> => {
    ...
    try {
      ...
      return typeof generated === 'string' ? generated.substring(0, 50) : 'equilibrado'
    } catch (err: unknown) {
  ...
  const handleGeneratePacing = async (): Promise<string> => {
    ...
    try {
      ...
      throw new Error(err instanceof Error ? err.message : 'Erro ao gerar ritmo')
    } finally {
      setGeneratingFor(null)
    }
  }
  ...
  const handleGenerateAudience = async (): Promise<string> => {
    ...
    try {
      ...
      throw new Error(err instanceof Error ? err.message : 'Erro ao gerar público-alvo')
    } finally {
      setGeneratingFor(null)
    }
  }
  ...
  const handleGenerateThemes = async (): Promise<string> => {
    ...
    try {
      ...
      throw new Error(err instanceof Error ? err.message : 'Erro ao gerar temas')
    } finally {
      setGeneratingFor(null)
    }
  }
