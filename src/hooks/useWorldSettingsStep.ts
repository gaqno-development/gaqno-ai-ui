...
} from "@/components/CreateBookWizard/WorldSettingsStep/types";
import { useWizardStepGeneration } from "@/hooks/useWizardStepGeneration";

export function useWorldSettingsStep({
  settings,
  onSettingsChange,
  bookContext,
}: IWorldSettingsStepProps) {
  ...
  const handleGenerateDescription = async (
    ...
    try {
      ...
      const generated =
        ...
        data?.summary ??
        `Uma descrição detalhada do cenário ${settingName}`;
  ...
  const handleGenerateAll = async () => {
    if (
      guardGenerateAll(
        bookContext,
        "Preencha pelo menos o título ou a premissa do livro antes de gerar cenários."
      )
    )
      return;
    await runWithGeneratingAll(async () => {
      try {
        ...
        else if (Array.isArray(context.settings))
          settingsArray = context.settings;
        else if (Array.isArray(context.locations))
        ...
        if (Array.isArray(settingsArray) && settingsArray.length > 0) {
          const newSettings: ISetting[] = (
            settingsArray as Array<Record<string, unknown>>
          ).map((s, idx) => {
          ...
          if (summary) {
            const newSettings: ISetting[] = [
              ...
              },
              {
                ...
                description:
                  "Um cenário importante para o desenvolvimento da narrativa.",
              },
              {
                id: `temp-${Date.now()}-3`,
                name: "Cenário de Conflito",
                description:
                  "O local onde os principais conflitos da história ocorrem.",
              },
            ];
            onSettingsChange(newSettings);
            addNotification({
              ...
              title: "Cenários gerados!",
              message:
                "3 cenários genéricos foram criados. Você pode editá-los conforme necessário.",
              duration: 3000,
            });
          } else {
            addNotification({
              ...
              message:
                "O blueprint não retornou cenários específicos. Adicione cenários manualmente.",
              duration: 5000,
            });
          }
        }
        ...
        addNotification({
          ...
          message:
            err instanceof Error
              ? err.message
              : "Não foi possível gerar os cenários automaticamente.",
          duration: 5000,
        });
      }
    });
