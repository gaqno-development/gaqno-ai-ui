...
import type { IBasicInfoStepProps } from "@/components/CreateBookWizard/BasicInfoStep/types";
import { GENRE_LABELS } from "@/components/CreateBookWizard/BasicInfoStep/types";
...
export function useBasicInfoStep({
  ...
  const handleGenerateTitle = async (): Promise<string> => {
    ...
    try {
      ...
      const generatedTitle =
        data?.blueprint?.title ?? data?.title ?? "Título Sugerido";
      return typeof generatedTitle === "string"
        ? generatedTitle
        : JSON.stringify(generatedTitle);
    } catch (err: unknown) {
      throw new Error(
        err instanceof Error ? err.message : "Erro ao gerar título"
      );
    } finally {
  ...
  const handleGeneratePremise = async (): Promise<string> => {
    ...
    try {
      ...
      const genre = selectedGenre ?? undefined;
      let prompt =
        "Gere uma premissa ou sinopse curta e envolvente para um livro";
      if (bookTitle) prompt += ` intitulado "${bookTitle}"`;
      if (genre) prompt += ` ${GENRE_LABELS[genre] ?? `do gênero ${genre}`}`;
      prompt +=
        ". A premissa deve incluir os elementos principais da história, personagens principais e o conflito central.";
      const data = await aiApi.generateBlueprint({
      ...
      const generatedPremise =
        ...
        data?.summary ??
        description ??
        "Uma premissa interessante para o livro";
      return typeof generatedPremise === "string"
        ? generatedPremise
        : JSON.stringify(generatedPremise);
    } catch (err: unknown) {
      throw new Error(
        err instanceof Error ? err.message : "Erro ao gerar premissa"
      );
    } finally {
      ...
      try {
        if (onGenerateCompleteBlueprint) {
          ...
          addNotification({
            ...
            message:
              "Todos os steps foram preenchidos com sucesso. Navegue pelos steps para revisar.",
            duration: 5000,
          });
        } else {
          const data = await aiApi.generateBlueprint({
            title:
              "Gere um livro completo com título, gênero e premissa envolvente",
            genre: "fiction",
            description:
              "Crie uma ideia original e criativa para um livro, incluindo título atraente, gênero apropriado e uma premissa detalhada.",
          });
          ...
          if (blueprint?.title) {
            const generatedTitle =
              typeof blueprint.title === "string"
                ? blueprint.title
                : JSON.stringify(blueprint.title);
          ...
          if (blueprint?.summary ?? blueprint?.description ?? data?.summary) {
            const generatedPremise =
              blueprint?.summary ?? blueprint?.description ?? data?.summary;
            const premise =
              typeof generatedPremise === "string"
                ? generatedPremise
                : JSON.stringify(generatedPremise);
