...
import type {
  IItem,
  IItemsStepProps,
} from "@/components/CreateBookWizard/ItemsStep/types";
...
export function useItemsStep({
  ...
  const handleGenerateItemDetails = async (
    ...
    try {
      ...
      const generated =
        ...
        data?.summary ??
        `Detalhes sobre ${field} do objeto ${itemName}`;
      return typeof generated === "string"
        ? generated
        : JSON.stringify(generated);
    } catch (err: unknown) {
      throw new Error(
        err instanceof Error ? err.message : "Erro ao gerar detalhes"
      );
    } finally {
  ...
  const handleGenerateAll = async () => {
    if (
      guardGenerateAll(
        bookContext,
        "Preencha pelo menos o título ou a premissa do livro antes de gerar itens."
      )
    )
      return;
      ...
      try {
        ...
        if (Array.isArray(itemsArray) && itemsArray.length > 0) {
          const newItems: IItem[] = (
            itemsArray as Array<Record<string, unknown>>
          ).map((item, idx) => ({
        ...
        addNotification({
          ...
          title: "Itens gerados!",
          message:
            "Os itens importantes foram gerados com sucesso. Você pode editá-los conforme necessário.",
          duration: 3000,
        });
      } catch (err: unknown) {
        addNotification({
          ...
          message:
            err instanceof Error
              ? err.message
              : "Não foi possível gerar os itens automaticamente.",
          duration: 5000,
        });
      }
    });
