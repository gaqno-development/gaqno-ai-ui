  ...
  if (parsed.startsWith("{")) {
    try {
      ...
      if (typeof obj === "object" && obj !== null) {
        const inner = obj.summary ?? obj.content ?? "";
        parsed = typeof inner === "string" ? inner : JSON.stringify(inner);
      }
    } catch {
...
export const useChapterEditor = (bookId: string, chapterId?: string) => {
  ...
  const { chapters, updateChapterAutoSave, updateChapter, refetch } =
  ...
  const shouldAutoGenerate =
    searchParams.get("generate") === "true" && !hasAutoGenerated.current;
  ...
  const handleGenerateContent = useCallback(async () => {
    ...
    try {
      ...
      const previousChapters = chapters
        .filter(
          (c) =>
            c.chapter_number < selectedChapter.chapter_number &&
            (c.content || c.notes)
        )
        .map((c) => ({
      ...
      const blueprintStructure = blueprint?.structure as
        | {
            chapters?: Array<{
              number?: number;
        ...
        | undefined;
      const blueprintChapter = blueprintStructure?.chapters?.find(
        (c) => (c.number ?? c.chapter_number) === selectedChapter.chapter_number
      );
        ...
        const analyzeData = await aiApi.analyzeContext({
          ...
          previousChapter: previousChapter
            ? {
                ...
                summary:
                  previousChapter.notes ??
                  previousChapter.content?.substring(0, 200) ??
                  undefined,
              }
            : undefined,
          ...
          currentChapterTitle:
            selectedChapter.title ?? blueprintChapter?.title ?? undefined,
          currentChapterSummary:
            selectedChapter.notes ?? blueprintChapter?.summary ?? undefined,
        });
      ...
      if (data.error && !(data as { mock?: unknown }).mock) {
        const msg =
      ...
      const generatedContent =
        data?.content ??
        (data as { mock?: { content?: string } })?.mock?.content ??
        "";
      const generatedTitle =
        data?.title ??
        (data as { mock?: { title?: string | null } })?.mock?.title ??
        null;
      const generatedSummary =
        data?.summary ??
        (data as { mock?: { summary?: string | null } })?.mock?.summary ??
        null;
  ...
  const pageInfo = selectedChapter
    ? formatPageInfo(selectedChapter.word_count, book?.genre)
    : "";
  const chapterPages = selectedChapter
    ? calculatePages(selectedChapter.word_count, book?.genre)
    : 0;
