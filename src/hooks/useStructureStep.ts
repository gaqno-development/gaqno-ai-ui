...
import type {
  IStructureStepProps,
  IStructureStepStructure,
} from "@/components/CreateBookWizard/StructureStep/types";
import { useWizardStepGeneration } from "@/hooks/useWizardStepGeneration";

export function useStructureStep({
  bookContext,
  onStructureChange,
  structure,
}: IStructureStepProps) {
  ...
  useEffect(() => {
    if (!structure) return;
    if (structure.plotSummary !== undefined)
      setPlotSummary(structure.plotSummary);
    if (structure.initialChapters !== undefined)
      setInitialChapters(structure.initialChapters);
    if (structure.mainConflict !== undefined)
      setMainConflict(structure.mainConflict);
  }, [
    structure?.plotSummary,
    structure?.initialChapters,
    structure?.mainConflict,
  ]);
  ...
  const handleGeneratePlotSummary = async (): Promise<string> => {
    ...
    try {
      ...
      throw new Error(
        err instanceof Error ? err.message : "Erro ao gerar resumo do enredo"
      );
    } finally {
  ...
  const handleGenerateChapters = async (): Promise<string> => {
    ...
    try {
      ...
      if (structureData?.chapters) {
        const chaptersText = (
          structureData.chapters as Array<{
            title?: string;
            number?: number;
            summary?: string;
          }>
          ...
          .map(
            (ch, idx) =>
              `${idx + 1}. ${ch.title ?? `Capítulo ${ch.number ?? idx + 1}`}: ${ch.summary ?? ""}`
          )
          .join("\n");
        setInitialChapters(chaptersText);
        onStructureChange?.({
          plotSummary,
          initialChapters: chaptersText,
          mainConflict,
        });
        return chaptersText;
      }
      ...
      onStructureChange?.({
        plotSummary,
        initialChapters: generated,
        mainConflict,
      });
      return generated;
    } catch (err: unknown) {
      throw new Error(
        err instanceof Error ? err.message : "Erro ao gerar capítulos"
      );
    } finally {
      setGeneratingFor(null);
    }
  ...
  const handleGenerateConflict = async (): Promise<string> => {
    ...
    try {
      ...
      throw new Error(
        err instanceof Error ? err.message : "Erro ao gerar conflito"
      );
    } finally {
      setGeneratingFor(null);
    }
  ...
  const handleGenerateAll = async () => {
    if (
      guardGenerateAll(
        bookContext,
        "Preencha pelo menos o título ou a premissa do livro antes de gerar a estrutura."
      )
    )
      return;
    await runWithGeneratingAll(async () => {
      try {
        ...
        const chArr = structureData.chapters as
          | Array<{ title?: string; number?: number; summary?: string }>
          | undefined;

        let newPlot = (structureData.plot_summary as string) ?? summary ?? "";
        let newChapters =
          (structureData.initial_chapters as string) ??
          (Array.isArray(chArr)
            ? chArr
                .map(
                  (ch, idx) =>
                ...
                .join("\n")
            : "");
        let newConflict = (structureData.main_conflict ??
          structureData.conflict ??
          (summary ? summary.substring(0, 200) : "")) as string;
        ...
        onStructureChange?.({
          plotSummary: newPlot,
          initialChapters: newChapters,
          mainConflict: newConflict,
        });
        ...
        addNotification({
          ...
          message:
            "Todos os campos da estrutura foram preenchidos com sucesso.",
          duration: 3000,
        });
      } catch (err: unknown) {
        addNotification({
          ...
          message:
            err instanceof Error
              ? err.message
              : "Não foi possível gerar a estrutura automaticamente.",
          duration: 5000,
        });
      }
    });
